...
const randomWords = (sourceArray, numItems) => {
    if (numItems > sourceArray.length) {
        throw new Error('number of items requested cannot be more than the length of the source array');
    }
...
const words = [
    ...new Set(originalText.toLowerCase().split(' ').map((word) => word.replace(/[^a-z]/g, ''))),
];

const argv = process.argv;

const numWords = (argv.length >= 3 && argv[2].match(/^[0-9]+$/g)) ? argv[2]:5;
